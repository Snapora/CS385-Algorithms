...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // TODO
        // The below instantiation will cause valgrind errors, place it after the cerr.
...
private:
    ...
    int diameter(Node<K, V> *node) const {
        ...
        return std::max(leftheight + rightheight + 3, std::max(leftdiameter, rightdiameter));
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        // TODO
        size_t sum_levels = 0;
        for (int i = 0, h = height(node) + 1; i < h; ++i) {
            size_t w = width(node, i);
            sum_levels += (w * i);
        }
        return sum_levels;
    }
